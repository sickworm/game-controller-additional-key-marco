...
启动器和配置中心打开时都会主动检测四个 XInput 槽位。AHK 会暂时停止转发、归零 vJoy，再写入一个短暂的四轴识别信号；只有经 XOutput 从 vJoy 输出的虚拟手柄会复现该信号。检测结束后 AHK 再次归零并恢复转发。未连接槽位和已确认的 XOutput 虚拟槽位会在页面中置灰，并在槽位名称后显示原因；只剩一个实体候选时会自动选择。存在多个实体候选时，点击“识别实体手柄（移动摇杆）”，并在 8 秒内大幅移动 GameSir 摇杆或按下 A，平台会根据实体输入活动选择槽位。不要在游戏运行中手工触发槽位检测，以免短暂探测状态被游戏读取。
